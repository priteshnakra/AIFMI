...
function IntelCard({ icon, label, color, data }) {
  ...
  if (!data) return null;
  return (
    <div style={{ background: '#fafafa', borderRadius: 8, border: '1px solid #e8e8e8', overflow: 'hidden' }}>
      <button onClick={() => setOpen(o => !o)} style={{
        width: '100%', background: 'none', border: 'none', cursor: 'pointer',
        padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10, textAlign: 'left',
      }}>
        <span style={{ fontSize: 14 }}>{icon}</span>
        <span style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 10, color, letterSpacing: 1, textTransform: 'uppercase', flex: 1 }}>{label}</span>
        <span style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 10, color: '#0a0a0a' }}>{open ? '▲' : '▼'}</span>
      </button>
      {open && (
        <div style={{ padding: '0 16px 14px', borderTop: `1px solid ${color}22` }}>
          <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', fontWeight: 600, fontSize: 13, color: '#0a0a0a', margin: '10px 0 6px' }}>{data.headline}</div>
          <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', fontSize: 12, color: '#0a0a0a', lineHeight: 1.7 }}>{data.detail}</div>
        </div>
      )}
    </div>
  );
...
function IntelPanel({ company, sector, prices, onClose }) {
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 'min(520px, 95vw)', background: '#f5f5f5', borderLeft: `1px solid ${sector?.color ?? '#1a1a2e'}`, zIndex: 201, display: 'flex', flexDirection: 'column', animation: 'slideIn 0.25s cubic-bezier(0.16,1,0.3,1)', overflowY: 'auto' }}>
        <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid #e8e8e8', background: '#fafafa', position: 'sticky', top: 0, zIndex: 10 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', fontWeight: 800, fontSize: 20, color: '#0a0a0a', marginBottom: 4 }}>{company.name}</div>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                {company.ticker && <span style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 11, color: sector?.color, background: `${sector?.color}18`, padding: '2px 8px', borderRadius: 4 }}>{company.ticker}</span>}
                <span style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 11, color: '#0a0a0a' }}>{company.exchange} · {company.hq}</span>
                {isPrivate && <span style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 10, color: '#0a0a0a', background: '#f0f0f0', padding: '2px 7px', borderRadius: 4 }}>PRIVATE</span>}
              </div>
            </div>
            <button onClick={onClose} style={{ background: '#ffffff', border: '1px solid #e8e8e8', color: '#0a0a0a', width: 32, height: 32, borderRadius: 6, cursor: 'pointer', fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>✕</button>
          </div>
          {priceData && (
            <div style={{ marginTop: 14, padding: '12px 16px', background: '#fafafa', borderRadius: 8, display: 'flex', gap: 24, alignItems: 'center' }}>
              <div>
                <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 22, fontWeight: 700, color: '#0a0a0a' }}>{fmt(priceData.price)}</div>
                <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 11, color: priceData.changePct >= 0 ? '#0FA97A' : '#DC3C3C', marginTop: 2 }}>{fmtPct(priceData.changePct)} today</div>
              </div>
        ...
        <div style={{ padding: '20px 24px', flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18 }}>
            <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 10, color: sector?.color, letterSpacing: 2, textTransform: 'uppercase' }}>AI Intelligence Briefing</div>
            <div style={{ flex: 1, height: 1, background: `${sector?.color}33` }} />
            {state === 'done' && <button onClick={fetchBriefing} style={{ background: 'none', border: '1px solid #e8e8e8', color: '#555555', fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 9, padding: '3px 8px', borderRadius: 4, cursor: 'pointer', letterSpacing: 1 }}>↻ REFRESH</button>}
          </div>
          {state === 'loading' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {['Querying financial intelligence...', 'Analyzing earnings data...', 'Synthesizing investor briefing...'].map((msg, i) => (
                <div key={i} style={{ padding: '14px 16px', background: '#fafafa', borderRadius: 8, animation: `pulse 1.5s ease-in-out ${i * 0.2}s infinite` }}>
                  <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 11, color: '#0a0a0a', marginBottom: 8 }}>{msg}</div>
          ...
          {state === 'error' && (
            <div style={{ padding: 20, background: '#1a0808', border: '1px solid #DC3C3C33', borderRadius: 8 }}>
              <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 11, color: '#DC3C3C', marginBottom: 8 }}>⚠ Failed to load briefing</div>
              <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 10, color: '#0a0a0a', marginBottom: 14 }}>{error}</div>
              <button onClick={fetchBriefing} style={{ background: '#DC3C3C18', border: '1px solid #DC3C3C44', color: '#DC3C3C', fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 10, padding: '6px 14px', borderRadius: 4, cursor: 'pointer' }}>Try again</button>
            </div>
          )}
          {state === 'done' && briefing && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14, animation: 'fadeIn 0.4s ease' }}>
              <div style={{ padding: 16, background: sentCfg.bg, border: `1px solid ${sentCfg.color}33`, borderRadius: 8 }}>
                <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 10, color: sentCfg.color, marginBottom: 10, letterSpacing: 1 }}>{sentCfg.label}</div>
                <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', fontSize: 13, color: '#0a0a0a', lineHeight: 1.7 }}>{briefing.summary}</div>
              </div>
              {[
              ...
              <div style={{ padding: '14px 16px', background: '#fafafa', borderRadius: 8, borderLeft: `3px solid ${sector?.color}` }}>
                <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 10, color: sector?.color, marginBottom: 8, letterSpacing: 1 }}>ANALYST OUTLOOK</div>
                <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', fontSize: 13, color: '#0a0a0a', lineHeight: 1.7, fontStyle: 'italic' }}>{briefing.outlook}</div>
              </div>
              <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 9, color: '#0a0a0a', textAlign: 'right' }}>Generated by Claude · Knowledge cutoff applies · Not financial advice</div>
            </div>
          )}
        </div>
      </div>
  ...
  if (watchlist.length === 0) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '80px 20px', gap: 16 }}>
        ...
        <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', fontWeight: 800, fontSize: 20, color: '#0a0a0a' }}>Your watchlist is empty</div>
        <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 11, color: '#0a0a0a', textAlign: 'center', lineHeight: 1.8 }}>
        ...
        <button onClick={() => onSelectSector('gpu')} style={{
          ...
          fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 11, padding: '10px 20px',
  ...
  return (
    <div style={{ animation: 'fadeIn 0.3s ease' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 24 }}>
        {[
          { label: 'Watching', value: watchlist.length, color: '#0a0a0a' },
          { label: 'Advancing', value: gainers, color: '#0FA97A' },
          { label: 'Declining', value: losers, color: '#DC3C3C' },
        ].map(({ label, value, color }) => (
          <div key={label} style={{ background: '#fafafa', borderRadius: 8, padding: '14px 18px', border: '1px solid #e8e8e8' }}>
            <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 10, color: '#0a0a0a', marginBottom: 6, letterSpacing: 1 }}>{label.toUpperCase()}</div>
            <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', fontWeight: 800, fontSize: 28, color }}>{value}</div>
          </div>
        ))}
      </div>
        ...
        return (
          <div key={sectorId} style={{ marginBottom: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
              ...
              <span style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 10, color: sector?.color, letterSpacing: 2, textTransform: 'uppercase' }}>
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 10, color: '#0a0a0a', marginTop: 2 }}>{company.spec}</div>
                    </div>

                    {/* Ticker */}
                    <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 11, color: sector?.color, background: `${sector?.color}18`, padding: '2px 8px', borderRadius: 4, flexShrink: 0 }}>
                    ...
                    <div style={{ textAlign: 'right', flexShrink: 0, minWidth: 80 }}>
                      {priceData ? (
                        <>
                          <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 14, fontWeight: 700, color: '#0a0a0a' }}>{fmt(priceData.price)}</div>
                          <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 10, color: up ? '#0FA97A' : '#DC3C3C' }}>{fmtPct(priceData.changePct)}</div>
                        </>
                      ) : (
                        <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 11, color: '#0a0a0a', fontStyle: 'italic' }}>Private</div>
                      )}
                    </div>

                    {/* Remove button */}
                    <button onClick={(e) => { e.stopPropagation(); onRemove(company.name); }} style={{
                      background: 'none', border: '1px solid #e8e8e8', color: '#0a0a0a',
                      width: 26, height: 26, borderRadius: 4, cursor: 'pointer',
                      fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 12,
                      display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
                    }}
                      onMouseEnter={e => { e.currentTarget.style.color = '#DC3C3C'; e.currentTarget.style.borderColor = '#DC3C3C44'; }}
                      onMouseLeave={e => { e.currentTarget.style.color = '#fafafa'; e.currentTarget.style.borderColor = '#1a1a2e'; }}
                    >✕</button>
                  </div>
                );
              })}
...
function StatusDot({ connected }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
      <span style={{ width: 8, height: 8, borderRadius: '50%', background: connected ? '#0FA97A' : '#fafafa', boxShadow: connected ? '0 0 6px #0FA97A' : 'none', display: 'inline-block', animation: connected ? 'pulse 2s ease-in-out infinite' : 'none' }} />
      <span style={{ color: connected ? '#0FA97A' : '#fafafa', fontSize: 11, fontFamily: '-apple-system, BlinkMacSystemFont, monospace' }}>{connected ? 'LIVE' : 'CONNECTING'}</span>
    </span>
  );
}
...
function PriceCell({ ticker, priceData, flashMap }) {
  if (!ticker || !priceData) return <div style={{ textAlign: 'right' }}><span style={{ color: '#0a0a0a', fontSize: 12, fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontStyle: 'italic' }}>Private</span></div>;
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'flex-end', background: flashColor, transition: 'background 0.4s', borderRadius: 4, padding: '2px 4px' }}>
      ...
      <div style={{ textAlign: 'right' }}>
        <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 13, fontWeight: 700, color: '#0a0a0a' }}>{fmt(priceData.price)}</div>
        <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 10, color: up ? '#0FA97A' : '#DC3C3C' }}>{fmtPct(priceData.changePct)}</div>
      </div>
    </div>
  );
...
function ProfileBtn({ ticker }) {
  ...
  return (
    <button
      ...
      style={{ fontSize: 11, fontWeight: 700, padding: '5px 10px', borderRadius: 6, border: '1.5px solid #1d4ed8', background: '#f0f0f0', color: '#1d4ed8', cursor: 'pointer', whiteSpace: 'nowrap' }}
    >Profile →</button>
  );
}
...
function CompanyRow({ company, rank, sectorColor, sectorId, prices, flashMap, onSelect, watchlist, onToggleWatch, isMobile }) {
  ...
  return (
    ...
    >
      <td style={{ padding: '10px 12px', color: sectorColor, fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 13, fontWeight: 700, width: 40 }}>{rank}</td>
      <td style={{ padding: '10px 12px' }} onClick={() => onSelect(company)}>
        <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', fontWeight: 600, fontSize: 14, color: '#0a0a0a' }}>
          {company.name}
          {isPrivate && <span style={{ marginLeft: 6, fontSize: 9, color: '#555555', fontFamily: '-apple-system, BlinkMacSystemFont, monospace', background: '#f0f0f0', padding: '1px 5px', borderRadius: 3 }}>PRIVATE</span>}
        </div>
        <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 10, color: '#0a0a0a', marginTop: 2 }}>{company.spec}</div>
      </td>
      <td style={{ padding: '10px 12px', fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 12, color: '#0a0a0a' }} onClick={() => onSelect(company)}>{company.ticker ?? '—'}</td>
      <td style={{ padding: '10px 12px', fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 11, color: '#0a0a0a' }} onClick={() => onSelect(company)}>{company.exchange}</td>
      <td style={{ padding: '10px 12px', fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 11, color: '#0a0a0a' }} onClick={() => onSelect(company)}>{company.hq}</td>
      ...
      <td style={{ padding: '10px 12px', width: 28, textAlign: 'center', color: '#555555', fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 14 }} onClick={() => onSelect(company)}>›</td>
    </tr>
  );
}
...
function OverviewCard({ sector, data, prices, onClick, active }) {
  ...
  return (
    <button onClick={onClick} style={{ background: active ? `${sector.color}18` : '#fafafa', border: `1px solid ${active ? sector.color : '#1a1a2e'}`, borderRadius: 10, padding: '16px 18px', cursor: 'pointer', textAlign: 'left', transition: 'all 0.2s', width: '100%' }}>
      <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', fontWeight: 800, fontSize: 15, color: sector.color, marginBottom: 8 }}>{sector.label}</div>
      <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 11, color: avgChange >= 0 ? '#0FA97A' : '#DC3C3C', marginBottom: 4 }}>Avg {fmtPct(avgChange)}</div>
      <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 10, color: '#0a0a0a' }}>{gainers}/{publicCos.length} advancing</div>
    </button>
  );
}
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: isMobile ? '20px 16px' : '28px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(5, 1fr) 160px', gap: 10, marginBottom: 28 }}>
          ...
          <button onClick={() => setActiveTab('watchlist')} style={{
            background: activeTab === 'watchlist' ? '#F0A50018' : '#fafafa',
            ...
            <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', fontWeight: 800, fontSize: 15, color: activeTab === 'watchlist' ? '#F0A500' : '#fafafa', marginBottom: 8 }}>⭐ Watchlist</div>
            <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 11, color: '#F0A500', marginBottom: 4 }}>{watchlist.length} stocks</div>
            <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 10, color: '#0a0a0a' }}>
        ...
        {isWatchlistTab ? (
          <>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20 }}>
              ...
              <span style={{ fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', fontWeight: 800, fontSize: 22, color: '#0a0a0a' }}>My Watchlist</span>
              <div style={{ height: 2, flex: 1, background: 'linear-gradient(90deg, #F0A50044, transparent)' }} />
              {watchlist.length > 0 && (
                <button onClick={() => { setWatchlist([]); saveWatchlist([]); }} style={{ background: 'none', border: '1px solid #e8e8e8', color: '#555555', fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 9, padding: '4px 10px', borderRadius: 4, cursor: 'pointer', letterSpacing: 1 }}>CLEAR ALL</button>
              )}
            </div>
            <div style={{ background: '#fafafa', borderRadius: 12, border: '1px solid #e8e8e8', padding: 24 }}>
              <WatchlistView
                watchlist={watchlist}
          ...
          <>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20 }}>
              ...
              <span style={{ fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', fontWeight: 800, fontSize: 22, color: '#0a0a0a' }}>{activeSector?.label} Sector</span>
              <div style={{ height: 2, flex: 1, background: `linear-gradient(90deg, ${activeSector?.color}44, transparent)` }} />
              <span style={{ fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 10, color: '#0a0a0a' }}>TOP 20 COMPANIES</span>
            </div>
            <div style={{ background: '#fafafa', borderRadius: 12, border: '1px solid #e8e8e8', padding: 24 }}>
              {SECTORS.map(s => (
        ...
        <div style={{ marginTop: 24, textAlign: 'center', fontFamily: '-apple-system, BlinkMacSystemFont, monospace', fontSize: 10, color: '#1a1a2a', lineHeight: 2 }}>
